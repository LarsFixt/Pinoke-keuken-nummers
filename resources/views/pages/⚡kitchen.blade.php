...
new class extends Component {
    ...
    {
        return [
            'echo:orders,OrderReady' => 'refreshCurrentNumber',
            'echo:orders,OrderCompleted' => 'refreshCurrentNumber',
        ];
    }
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                @forelse($this->readyOrders as $order)
                    <button wire:click="completeOrder({{ $order->id }})"
                        class="p-4 bg-lime-100 dark:bg-lime-900/30 border border-lime-200 dark:border-lime-800 rounded-xl text-center hover:bg-lime-200 dark:hover:bg-lime-900/50 transition relative group group-active:scale-95">
